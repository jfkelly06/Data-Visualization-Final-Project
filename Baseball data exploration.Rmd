...
cor.test(baseball_dat2$BMI, log10(baseball_data$HR), method = 'pearson')
...
ggplot(aes(x = BMI, y = log10(power), size = 2),
       data = baseball_data) +
  geom_point(aes(color = handedness),
...
In this section, I created the same plots as those in the bivariate plots section, but included the handedness variable. I had hoped the superiority of lefties and inferiority of switch hitters would be more clear, but unfortunately it was not.

#Export to CSV
write.csv(baseball_data, file = "baseball_data.csv", row.names=FALSE)